 ...
 * SPI1 Interface
 */
void PIOS_SPI_flexiio_irq_handler(void);
void DMA2_Stream0_IRQHandler(void) __attribute__((alias("PIOS_SPI_flexiio_irq_handler")));
void DMA2_Stream5_IRQHandler(void) __attribute__((alias("PIOS_SPI_flexiio_irq_handler")));
static const struct pios_spi_cfg pios_spi_flexiio_cfg = {
    .regs  = SPI1,
    .remap = GPIO_AF_SPI1,
    .init  = {
        .SPI_Mode              = SPI_Mode_Master,
...
static uint32_t pios_spi_flexiio_id;
void PIOS_SPI_flexiio_irq_handler(void)
{
    /* Call into the generic code to handle the IRQ for this specific device */
    PIOS_SPI_IRQ_Handler(pios_spi_flexiio_id);
}
